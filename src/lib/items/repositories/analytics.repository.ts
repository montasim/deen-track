...
import { SellPostStatus, ItemCondition } from '@prisma/client'
...
export async function recordSellPostView(data: {
    ...
    return prisma.itemSellPostView.create({
        data: {
...
export async function getSellPostViewsOverTime(
    ...
    const views = await prisma.itemSellPostView.groupBy({
        by: ['visitedAt'],
        where: {
            sellPostId,
...
export async function getMarketplaceStats() {
    const [
        ...
        prisma.itemSellPost.aggregate({
            where: { status: SellPostStatus.AVAILABLE },
            _sum: { price: true }
        }),
        prisma.itemOffer.count(),
        prisma.itemOffer.count({
...
export async function getPostsByConditionDistribution() {
    const distribution = await prisma.itemSellPost.groupBy({
        by: ['condition'],
        where: {
...
export async function getPostsOverTime(days = 30) {
    ...
    const posts = await prisma.itemSellPost.groupBy({
        by: ['createdAt'],
        where: {
...
export async function getSalesOverTime(days = 30) {
    ...
    const sales = await prisma.itemSellPost.groupBy({
        by: ['soldAt'],
        where: {
...
export async function getTopSellers(limit = 10) {
    const sellers = await prisma.itemSellPost.groupBy({
        by: ['sellerId'],
        where: { status: SellPostStatus.SOLD },
        _count: { id: true },
        _sum: { price: true },
...
export async function getSellerAnalytics(sellerId: string) {
    const [
        ...
        prisma.itemSellPost.count({ where: { sellerId } }),
        prisma.itemSellPost.count({
