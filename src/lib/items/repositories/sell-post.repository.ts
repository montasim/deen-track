 ...
 * This module handles all database operations for the ItemSellPost model
 */

import { prisma } from '@/lib/prisma'
import { ItemCondition, SellPostStatus } from '@prisma/client'
...
export async function getSellPosts(
    ...
    const [posts, total] = await Promise.all([
        ...
        prisma.itemSellPost.count({ where }),
    ])
...
export async function getSellPostById(id: string) {
    return prisma.itemSellPost.findUnique({
        where: { id },
        include: {
            seller: {
                select: {
...
/**
 * Get related sell posts (similar items or in same city)
 */
export async function getRelatedSellPosts(
    sellPostId: string,
    limit = 6
) {
    const post = await prisma.itemSellPost.findUnique({
        where: { id: sellPostId },
        select: { city: true }
    })
    ...
    return prisma.itemSellPost.findMany({
        where,
        include: {
            seller: {
                select: {
...
export async function createSellPost(data: CreateSellPostData) {
    return prisma.itemSellPost.create({
        data: {
            title: data.title,
...
export async function updateSellPost(
    ...
    return prisma.itemSellPost.update({
        where: { id },
        data,
        include: {
            seller: {
...
export async function markSellPostAsSold(id: string, sellerId: string) {
    return prisma.itemSellPost.updateMany({
        where: {
            id,
            sellerId
        },
...
export async function permanentlyDeleteSellPost(id: string) {
    return prisma.itemSellPost.delete({
        where: { id }
    })
}
