 ...
 * This module handles all database operations for the ItemOffer model
 */
...
export async function getOfferById(id: string) {
    return prisma.itemOffer.findUnique({
        where: { id },
        include: {
            sellPost: {
                include: {
...
export async function createOffer(data: CreateOfferData) {
    ...
    const sellPost = await prisma.itemSellPost.findUnique({
    ...
    return prisma.itemOffer.create({
        data: {
            sellPostId: data.sellPostId,
            buyerId: data.buyerId,
...
export async function respondToOffer(
    ...
    const offer = await prisma.itemOffer.findUnique({
        where: { id: offerId },
        include: {
            sellPost: {
    ...
    if (data.status === 'ACCEPTED') {
        await prisma.itemOffer.updateMany({
            where: {
        ...
        await prisma.itemSellPost.update({
            where: { id: offer.sellPostId },
            data: { status: 'PENDING' }
        })
    }

    return prisma.itemOffer.update({
        where: { id: offerId },
        data: updateData,
        include: {
            buyer: {
...
export async function deleteOffer(offerId: string) {
    return prisma.itemOffer.delete({
        where: { id: offerId }
    })
}
...
export async function getSellPostOfferStats(sellPostId: string) {
    const [total, pending, accepted, rejected, countered] = await Promise.all([
        prisma.itemOffer.count({ where: { sellPostId } }),
        prisma.itemOffer.count({ where: { sellPostId, status: OfferStatus.PENDING } }),
        prisma.itemOffer.count({ where: { sellPostId, status: OfferStatus.ACCEPTED } }),
        prisma.itemOffer.count({ where: { sellPostId, status: OfferStatus.REJECTED } }),
        prisma.itemOffer.count({ where: { sellPostId, status: OfferStatus.COUNTERED } }),
    ])
