...
export default function SettingsConnectedAccounts() {
  ...
  useEffect(() => {
    ...
    if (connected) {
      ...
      // Clean up the URL
      router.replace('/dashboard/settings/connected-accounts', { scroll: false })
    }
  }, [searchParams, router])
  ...
  const handleProviderToggle = async (provider: Provider, checked: boolean) => {
    if (checked) {
      // Trigger OAuth flow directly without redirecting to sign-in page
      try {
        const response = await fetch(`/api/auth/oauth/${provider.toLowerCase()}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            redirect: '/dashboard/settings/connected-accounts',
  ...
  return (
    <ContentSection
      ...
      desc="Manage your social login accounts for quick sign-in."
    >
      <Card>
