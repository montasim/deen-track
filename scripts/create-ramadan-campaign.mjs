...
if (!databaseUrl) {
  ...
  console.error('  DATABASE_URL="your-database-url" pnpm run script:create-ramadan-campaign')
  process.exit(1)
}
...
// User ID who will be the creator (you can change this)
const CREATOR_USER_ID = process.env.CREATOR_USER_ID || 'default-admin-user-id'

// Ramadan Campaign Data from the JSON
  ...
  try {
      ...
      template.templateTasks.map((tt) =>
        prisma.campaignTask.create({
          data: {
            ...
            startDate: new Date(ramadanCampaignData.startDate),
            endDate: new Date(ramadanCampaignData.endDate),
            validationType: 'MANUAL',
            isActive: true,
            entryById: CREATOR_USER_ID,
            // Create achievements for this task
            achievements: {
              create: (tt.achievementsTemplate || []).map((ach) => ({
                name: ach.name,
    ...
    const campaign = await prisma.gamifiedCampaign.create({
      data: {
        ...
        isActive: true,
        rewards: ramadanCampaignData.rewards,
        entryById: CREATOR_USER_ID,
    ...
    campaign.tasks.forEach((ct, index) => {
      ...
      console.log(`      - Points: ${template.templateTasks[index].points}`)
